...
public interface DeviceMapper {
	...
	@Select("SELECT * FROM (SELECT * FROM device WHERE d_seq=#{d_seq})a, deviceinfo b WHERE a.d_seq=b.d_seq AND di_seq=#{1.di_seq}") // 해당 기기 상세정보 확인
	public DeviceCommand deviceInfo(Integer d_seq, Integer di_seq);
	@Select("SELECT * FROM deviceinfo WHERE d_seq=#{d_seq},di_seq=#{di_seq}")
